...
public class RatingTankActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void getUserInfo() {
        handler.post(new Runnable() {
            @Override
            public void run() {
                progressDialog = new ProgressDialog(RatingTankActivity.this);
                progressDialog.setMessage(getString(R.string.loading));
                progressDialog.show();
            }
        });
        InternetConnectionUtils.isInternetAvailable(RatingTankActivity.this, new InternetAvailableCallback() {
            ...
            public void onInternetAvailable(boolean isAvailable) {
                if (isAvailable) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            handler.post(new Runnable() {
                                ...
                                public void run() {
                                    progressDialog.hide();
                                    AppErrorsManager.showErrorDialog(RatingTankActivity.this, e.getMessage());
                                }
                            });
                        }
                        if (!response.isSuccessful()) {
                            handler.post(new Runnable() {
                                ...
                                public void run() {
                                    progressDialog.hide();
                                    AppErrorsManager.showErrorDialog(RatingTankActivity.this, response + "");
                                }
                            });
                        }
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            progressDialog.hide();
                            AppErrorsManager.showErrorDialog(RatingTankActivity.this, getString(R.string.error_network));
                        }
                    });
                }
            }
        ...
        InternetConnectionUtils.isInternetAvailable(this, new InternetAvailableCallback() {
            ...
            public void onInternetAvailable(boolean isAvailable) {
                if (isAvailable) {
                    ...
                    okhttp3.Request request = new okhttp3.Request.Builder().url(FontManager.URL
                            + "rating_tank_partner").post(requestBody).build();
                    final okhttp3.Response response;
                    try {
                        response = client.newCall(request).execute();
                        String response_data = Objects.requireNonNull(response.body()).string();
