...
public class RatingTripActivity extends AppCompatActivity implements View.OnClickListener {
    private TextView mobileTxt, nameTxt;
    private String user_id, guid, id, trip_name;
    ...
    public void getUserInfo() {
        handler.post(new Runnable() {
            @Override
            public void run() {
                progressDialog = new ProgressDialog(RatingTripActivity.this);
                progressDialog.setMessage(getString(R.string.loading));
                progressDialog.show();
            }
        });
        InternetConnectionUtils.isInternetAvailable(RatingTripActivity.this, new InternetAvailableCallback() {
            ...
            public void onInternetAvailable(boolean isAvailable) {
                if (isAvailable) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            handler.post(new Runnable() {
                                ...
                                public void run() {
                                    progressDialog.hide();
                                    AppErrorsManager.showErrorDialog(RatingTripActivity.this, e.getMessage());
                                }
                            });
                        }
                        if (!response.isSuccessful()) {
                            handler.post(new Runnable() {
                                ...
                                public void run() {
                                    progressDialog.hide();
                                    AppErrorsManager.showErrorDialog(RatingTripActivity.this, response + "");
                                }
                            });
                        }
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            progressDialog.hide();
                            AppErrorsManager.showErrorDialog(RatingTripActivity.this, getString(R.string.error_network));
                        }
                    });
                }
            }
        ...
        if (startNumber == 0) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (TextUtils.equals(trip_name, "null"))
                        addBoatInfo(requestBody);
                    else
                        addBoatInfo_(requestBody);
                }
            }).start();
        }
    }
        ...
        InternetConnectionUtils.isInternetAvailable(this, new InternetAvailableCallback() {
            ...
            public void onInternetAvailable(boolean isAvailable) {
                if (isAvailable) {
                    ...
                    okhttp3.Request request = new okhttp3.Request.Builder().url(FontManager.URL
                            + "rating_boat_trip_partner").post(requestBody).build();
                    final okhttp3.Response response;
                    try {
                        response = client.newCall(request).execute();
                        String response_data = Objects.requireNonNull(response.body()).string();
    ...
    public void addBoatInfo_(final RequestBody requestBody) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                progressDialog = new ProgressDialog(RatingTripActivity.this);
        ...
        InternetConnectionUtils.isInternetAvailable(this, new InternetAvailableCallback() {
            ...
            public void onInternetAvailable(boolean isAvailable) {
                if (isAvailable) {
                    ...
                    client = builder.build();
                    okhttp3.Request request = new okhttp3.Request.Builder().url(FontManager.URL
                            + "rating_trip_partner").post(requestBody).build();
                    final okhttp3.Response response;
                    try {
                        response = client.newCall(request).execute();
                        String response_data = Objects.requireNonNull(response.body()).string();
